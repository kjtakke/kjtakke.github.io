...
Sub aggTable(ByVal aggStr As String)
    ...
    tmpStr = aggVar(0)
    output = aggregate.aggTableCols(tmpStr, UBound(args) + 1, args)
...
Function aggTableCols(tbl As String, cols, aggtype) As Variant
    Dim counter As Long
    ...
    'Unique Values
    uWDTable = aggregate.aggUniqueValues(TempAry, UBound(WDTable), colsTmp)
    ...
    aggTableCols = uWDTable
End Function

Function aggUniqueValues(index As Variant, rows As Integer, cols As Long) As Variant
